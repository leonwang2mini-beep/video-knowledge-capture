...
- 维护者本机的跨项目协作规则放在被 Git 排除的 `.agents/local/`，根 `AGENTS.md` 只负责发现该目录；社区检出不依赖这些文件，也不得把个人路径写入公开仓库。
...
- 2026-08-27：M11 外部 Beta 反馈统一使用 `doctor:shareable`；该模式脱敏配置、用户目录、Hermes Home 和 Inbox 路径，但测试者发布前仍必须人工复核，且不得自行补充链接、凭据、媒体、转写内容或原始下载日志。
- 外部宿主 E4 必须由首次用户亲自完成真实宿主发现、`completed`、Markdown 内容回读和同链接 `duplicate`，再明确确认；Hermes 手机 E4 还要求从真实手机消息发起。CI、临时目录模拟、本机插件调用或成功提示均不能替代。
- M11 本地发布门禁使用干净克隆执行 `npm.cmd run verify:public-release`；维护者工作区中被 Git 忽略的宣传素材不是公共树审计输入，不得为了让审计通过而删除这些本地产物。
